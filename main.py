...
group_train.add_argument('--training_sample_ratio', type=float, default=1.0,
                    help='training sample ratio')
group_train.add_argument('--re_ratio', type=int, default=5,
                    help='multiple of of data augmentation')
group_train.add_argument('--sampling_mode', type=str, default='kmeans', choices=['random', 'kmeans'],
...
def experiment(log_dir = ''):
    ...
    text_features = text_cache.text_features.to(device)
    
    print("PMGDG device:", device)
    ...
    if args.sampling_mode == 'kmeans':
        ...
        for label, indices in class_indices.items():
            if len(indices) <= samples_per_class:
                ...
                # Select nearest neighbor for each centroid greedily ensuring uniqueness
                selected_for_class = []
                for center in kmeans.cluster_centers_:
                    ...
                    for local_idx in sorted_indices:
                        ...
                        if global_idx not in selected_for_class:
                            selected_for_class.append(global_idx)
                            break
                selected_indices += selected_for_class
    else:
        for label, indices in class_indices.items():
    ...
    g1 = Generator(imdim=N_BANDS, patch_size = hyperparams['patch_size'],layers = layers, dim1 = args.dim1, dim2 = args.dim2, device=device, text_dim=512).to(device)
    g2 = Generator(imdim=N_BANDS, patch_size = hyperparams['patch_size'],layers = layers, dim1 = args.dim1, dim2 = args.dim2, device=device, text_dim=512).to(device)
    d1 = Dis(imdim=N_BANDS, patch_size = hyperparams['patch_size'],layers = layers, proj=args.pro_dim, num_classes=num_classes).to(device)
    d2 = Dis(imdim=N_BANDS, patch_size = hyperparams['patch_size'],layers = layers, proj=args.pro_dim, num_classes=num_classes).to(device)
    ...
    if not args.skip_stage1:
      ...
      for epoch in range(pre_epoch):
          ...
          teacc1, res1 = evaluate_pre(g1, d1, train_loader, device)
          teacc2, res2 = evaluate_pre(g2, d2, train_loader, device)
    ...
    D_net = discriminator(inchannel=N_BANDS, outchannel=args.pro_dim, num_classes=num_classes, patch_size=hyperparams['patch_size']).to(device)
    ...
    for epoch in range(1,args.max_epoch+1):
        ...
        D_net.eval()
        D_net.mode = 'test'
        taracc, results = evaluate(D_net, test_loader, device)
        if best_acc < taracc:
            best_acc = taracc
    ...
    try:
        ...
        save_run_tsne(g1, g2, D_net, train_loader, device, plots_dir, args.source_name)
    except Exception as e:
